...
/// with the N-API, which, being garbage collected, cannot easily obtain
...
impl<T> BuilderWrapper<T> {
    ...
    pub fn consume<O>(&self, mod_fn: impl FnOnce(T) -> O) -> napi::Result<O> {
        let mut inner = self.inner.lock().unwrap();
        let inner_builder = inner.take().ok_or_else(|| {
            napi::Error::from_reason(format!("{} has already been consumed", self.name))
        })?;
