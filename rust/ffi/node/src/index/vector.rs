...
use lancedb::index::IvfPqIndexBuilder;
...
pub fn table_create_vector_index(mut cx: FunctionContext) -> JsResult<JsPromise> {
    ...
    let tbl = table.clone();
    let mut index_builder = tbl.create_index().column(column_name);
    if let Some(replace) = replace {
        index_builder = index_builder.replace(replace);
    }

    let ivf_pq_builder = index_builder.vector().ivf_pq();
    let ivf_pq_builder =
        get_index_params_builder(&mut cx, index_params, ivf_pq_builder).or_throw(&mut cx)?;

    rt.spawn(async move {
        let idx_result = ivf_pq_builder.execute().await;
...
fn get_index_params_builder(
    ...
    if let Some(_) = obj.get_opt::<JsString, _, _>(cx, "index_name")? {
